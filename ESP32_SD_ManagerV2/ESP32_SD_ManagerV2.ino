...
const char* ssid     = "ESP32_SD_ManagerV0.2";
...
// Disable brownout detector (optional)
    ...
    while (file) {
      ...
      if (!file.isDirectory()) {
        ...
        if (lowerName.endsWith(".txt") || lowerName.endsWith(".csv") || lowerName.endsWith(".log") || 
            ...
            lowerName.endsWith(".js") || lowerName.endsWith(".css") || lowerName.endsWith(".md") ||
...
void setup(){
  Serial.begin(115200); 
  if(DEBUG) Serial.println("[DEBUG] Starting ESP32 SD Manager");
 ...
 // disableBrownout();
  ...
  if(DEBUG) Serial.println("[DEBUG] SD card initialized successfully");
  ...
  server.begin(); 
  if(DEBUG) Serial.println("[DEBUG] HTTP server started successfully");
}

void loop(){ 
  server.handleClient(); 
  ensureAP(); 
  yield(); // Give system time to handle other tasks
}